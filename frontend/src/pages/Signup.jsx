...
import { Shield, Mail, Lock, User as UserIcon, Hexagon, Eye, EyeOff } from 'lucide-react';
  ...
  return (
    <div className="page-container" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '80vh' }}>
      <FadeInSection direction="up">
        <div className="glass auth-card" style={{ width: '100%', maxWidth: '450px', padding: 'var(--auth-padding, 3rem 2rem)', borderRadius: '20px' }}>
          <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
            ...
            <p style={{ color: 'var(--text-muted)' }}>Join the White Zero OSINT network.</p>
          </div>

          {errorMsg && (
            <div style={{ background: 'rgba(2ef, 68, 68, 0.1)', border: '1px solid #ef4444', color: '#ef4444', padding: '0.75rem', borderRadius: '8px', marginBottom: '1.5rem', textAlign: 'center' }}>
              {errorMsg}
            </div>
          )}
          ...
          <form onSubmit={handleSubmit}>
            <div className="form-group" style={{ marginBottom: '1.5rem' }}>
              ...
              <div style={{ position: 'relative', display: 'flex', alignItems: 'center' }}>
                <UserIcon size={20} style={{ position: 'absolute', left: '15px', color: 'var(--text-muted)' }} />
                <input
                  type="text"
          ...
          <p style={{ textAlign: 'center', marginTop: '2rem', color: 'var(--text-muted)' }}>
            Already have an account? <Link to="/login" style={{ color: '#00d2ff', textDecoration: 'none', fontWeight: '600' }}>Sign In</Link>
          </p>
        </div>
      </FadeInSection>
    </div>
