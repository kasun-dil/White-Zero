    ...
    <div style={{ minHeight: '100vh', background: 'radial-gradient(circle at 50% 0%, #0a1128 0%, #050505 100%)', width: '100%', position: 'relative' }}>
      <div className="page-container" style={{ paddingTop: '100px', paddingBottom: '60px', maxWidth: '1400px', margin: '0 auto' }}>
        <div className="container" style={{ maxWidth: '1200px', margin: '0 auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '350px 1fr', gap: '2rem', alignItems: 'start' }} className="reports-grid-layout">

            {/* Sidebar - Case History */}
            <div className="glass fade-in thin-scrollbar" style={{ 
              borderRadius: '24px', 
              padding: '1.5rem', 
                ...
                {reports.length > 0 ? [...reports].sort((a, b) => {
                  ...
                  <div
                    ...
                    style={{
                      ...
                      borderRadius: '16px',
                      cursor: 'pointer',
                      background: selectedReport?._id === r._id ? 'rgba(59, 130, 246, 0.15)' : 'rgba(255,255,255,0.02)',
                      border: '1px solid',
                      borderColor: selectedReport?._id === r._id ? 'rgba(255, 255, 255, 0.72)' : 'rgba(255,255,255,0.05)',
                      position: 'relative',
                      opacity: r.isClosed ? 0.7 : 1
                    }}
                  >
                    {!r.isReadByUser && (
                      <div style={{ position: 'absolute', top: '10px', right: '10px', width: '8px', height: '8px', background: '#3b82f6', borderRadius: '50%', boxShadow: '0 0 10px #3b82f6' }} />
                    )}
                    <h3 style={{ fontSize: '1rem', fontWeight: '700', marginBottom: '0.4rem', color: selectedReport?._id === r._id ? '#3b82f6' : 'white' }}>{r.title}</h3>
            ...
            <div className="fade-in thin-scrollbar" style={{ 
              height: 'calc(100vh - 240px)', 
              overflowY: 'auto' 
            }}>
              {selectedReport ? (
                <div className="glass" style={{ borderRadius: '24px', overflow: 'hidden' }}>
                  {/* Case Status Bar */}
                  <div style={{ padding: '1.5rem 2rem', background: 'rgba(59, 130, 246, 0.08)', borderBottom: '1px solid rgba(255,255,255,0.05)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div>
                      <h2 style={{ fontSize: '1.5rem', fontWeight: '800', margin: 0 }}>{selectedReport.title}</h2>
                      <p style={{ fontSize: '0.85rem', color: '#888', margin: 0 }}>Reference: <span style={{ color: '#3b82f6', fontWeight: 'bold' }}>{selectedReport.referenceId}</span> | Opened: {new Date(selectedReport.createdAt).toLocaleDateString()}</p>
                    </div>
                        ...
                        <div key={i} className="message-animation" style={{
                          ...
                          <div style={{ display: 'flex', justifyContent: 'space-between', gap: '2rem', marginBottom: '0.6rem' }}>
                            <span style={{
                              fontSize: '0.65rem',
                              fontWeight: 'bold',
                              color: resp.role === 'user' ? '#3b82f6' : '#10b981',
                              letterSpacing: '1px'
                            }}>
